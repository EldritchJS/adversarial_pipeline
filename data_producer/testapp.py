...
class DatabaseLoader:
    ...
    def __init__(self):
        ...
        self.cleartables = '1' #environ.get("CLEARTABLES")
...
#    logging.info('finished creating kafka producer')
    ...
    while True:
        ...
        try:
            cur.execute('select * from images where status=%s;', ('Unprocessed',))
            res = cur.fetchall()
        except Exception:
            res = []
        cur.close()
...
def parse_args(parser):
    ...
    os.environ['DBHOST'] = args.dbhost
    os.environ['DBNAME'] = args.dbname
    os.environ['DBUSERNAME'] = args.dbusername
    os.environ['DBPASSWORD'] = args.dbpassword
    os.environ['CLEARTABLES'] = args.cleartables
    return args
